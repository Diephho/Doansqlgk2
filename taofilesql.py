...
extra_files = ['Sở.xlsx', 'Phòng.xlsx', 'Loại hình.xlsx', 'Loại trường.xlsx']

extra_file_name = 'table_phu.sql'

with open(os.path.join(folder_path, extra_file_name), 'w', encoding='utf-16') as f:
    f.write("USE truonghoc;" + "\n")

for file in extra_files:
    df_ef = pd.read_excel(os.path.join(folder_path, file), sheet_name='Sheet1')
    with open(os.path.join(folder_path, extra_file_name), 'a', encoding='utf-16') as f:        
        if file == 'Sở.xlsx':
        ...
        if file == 'Phòng.xlsx':
            for index, row in df_ef.iterrows():
                insert_sql = f"INSERT INTO PHONG_GD_DT (MA_PHONG, TEN_PHONG) VALUES ('{row['Mã Phòng']}', '{row['Tên Phòng']}');\n"
                f.write(insert_sql)
...
for file in file_list:
    ...
    with open(os.path.join(folder_path, sql_file_name), 'w', encoding='utf-16') as f:
        f.write("USE truonghoc;" + "\n")
        ...
        for index, row in df.iterrows():
            ...
            # Tạo câu lệnh INSERT
            newRow = row
            insert_sql = f"INSERT INTO TRUONG (MATRUONG, TENTRUONG, MASO, MAPHONG, DIACHI, MA_LOAI_HINH, MA_LOAI_TRUONG) VALUES ('{newRow['Mã trường']}', '{newRow['Tên trường']}', '{newRow['Sở GD&ĐT']}', '{newRow['Phòng GD&ĐT']}', '{newRow['Địa chỉ']}', {newRow['Loại hình']}, {newRow['Loại trường']});\n"
            insert_sql.replace("nan", "null")
